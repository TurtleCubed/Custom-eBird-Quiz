...
def save_img(ml_num, rating, qual=75, redown=False):
    ...
    try:
        ...
        img.convert("RGB").save(f'PhotoLibrary/{ml_num}_{rating}.jpg', format='JPEG', quality=qual)
    except UnidentifiedImageError as e:
        print(ml_num)

if __name__=="__main__":
    from multiprocessing import Pool, freeze_support
    nums, ratings = read_csv('sort_by_ML.csv')
    freeze_support()
    with Pool(8) as pool:
        pool.starmap(save_img, zip(nums, ratings))
