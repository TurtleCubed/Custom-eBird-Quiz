...
class Cub(Dataset):
    ...
    def __init__(
        ...
        self._dataset_folder = pathlib.Path(root)
    ...
    def __getitem__(self, idx):
        ...
        if self._transform is not None:
            ...
            sample = {
                ...
                "bbox": np.array([sample["x_b"], sample["y_b"], sample["w"], sample["h"]])
            }
        ...
        if not self._as_pts:
            ...
            n_pix_per_cell_h = sample["image"].shape[0] // 7
            n_pix_per_cell_w = sample["image"].shape[1] // 7
            ...
            part_loc_remapped = [self._parts_index_remap[sample["part_ids"][i]] for i in range(len(sample["part_ids"]))]
            # parts = zip(part_loc_remapped, sample["part_locs"])

        output = {
            "image": sample["image"],
            "target": target,
            "part_locs": sample["part_locs"],
            "part_ids": part_loc_remapped,
            "bbox": sample["bbox"]
        }
