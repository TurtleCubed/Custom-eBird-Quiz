...
trans = transforms.Compose([
    transforms.RandomSizedBBoxSafeCrop(224, 224), 
    transforms.HorizontalFlip(),
    # transforms.ToTensor(),
    normalize
], bbox_params=transforms.BboxParams(format="coco", label_fields=["bbox_classes"]),
  keypoint_params=transforms.KeypointParams(format="xy", label_fields=["keypoints_classes"]),
)
test_trans = transforms.Compose([
    transforms.SmallestMaxSize(224),
...
trainset = Cub("./birdie/CUB_200_2011", train=True, transform=trans, return_image_only=False)
train_dl = torch.utils.data.DataLoader(
    trainset, batch_size=32, shuffle=True#, num_workers=8
    )

testset = Cub("./birdie/CUB_200_2011", train=False, transform=test_trans, return_image_only=False)
